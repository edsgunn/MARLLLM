...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--config', default=None, help='Optional YAML defaults file (same format as training configs).')
    p.add_argument('--model', default='Qwen/Qwen2.5-1.5B', help='HuggingFace model used to instantiate both agents.')
    p.add_argument('--checkpoint', default=None, help='Path to a Trainer checkpoint (.pt). Omit to evaluate the pretrained baseline (Phase A condition 1).')
    p.add_argument('--checkpoint-1', default=None, help='Optional separate checkpoint for agent_1. If omitted, agent_1 uses the same checkpoint as agent_0 (or pretrained).')
    p.add_argument('--device', default=None)
    p.add_argument('--device-0', default=None)
    p.add_argument('--device-1', default=None)
    p.add_argument('--dtype', default='bfloat16', help='torch_dtype for model load: bfloat16 / float16 / float32 / auto')
    p.add_argument('--attn-impl', default=None)
    p.add_argument('--n-episodes', type=int, default=64, help='Number of evaluation episodes to roll out.')
    ...
    p.add_argument('--seed', type=int, default=10000, help='Eval seed — kept disjoint from training seeds (default 10_000) so the eval scenarios are not the ones the agent trained on.')
    ...
    p.add_argument('--output', default=None, help="Where to write the JSON summary. Defaults to <checkpoint dir>/eval.json if --checkpoint is given, else 'eval.json' in the cwd.")
    p.add_argument('--save-trajectories', action='store_true', help='Also save per-episode token-level trajectories for downstream behavioural analysis (Phase A §3.2).')
    if pre_args.config:
...
def main() -> None:
    ...
    from marlllm.dialogue import DialogueContext, chat_eos_token_ids, verify_special_tokens
    ...
    agent_0 = IndependentAgent(agent_id='agent_0', character_prompt=args.prompt_0, model_name_or_path=args.model, device=device_0, torch_dtype=torch_dtype, attn_implementation=args.attn_impl)
    print(f'Loading agent_1: {args.model}  →  {device_1}')
    agent_1 = IndependentAgent(agent_id='agent_1', character_prompt=args.prompt_1, model_name_or_path=args.model, device=device_1, torch_dtype=torch_dtype, attn_implementation=args.attn_impl)
    if args.checkpoint:
    ...
    env = DealOrNoDealEnv(tokenizer=agent_0.tokenizer, max_dialogue_turns=args.dialogue_turns, action_token_budget=args.token_budget, env_token_budget=args.env_token_budget, seed=args.seed, role_shuffle=args.role_shuffle)
    ...
    for ep in range(args.n_episodes):
        ...
        rng_seed += 1
        contexts: dict[str, DialogueContext] = {aid: DialogueContext(tokenizer=agents[aid].tokenizer, system_prompt=character_prompts[aid]) for aid in agents}
        for agent_id in env.agent_iter():
            obs, _r, term, trunc, info = env.last()
            obs_text = obs if isinstance(obs, str) else agent_0.tokenizer.decode(obs, skip_special_tokens=True) if obs else ''
            if obs_text:
            ...
            with torch.no_grad():
                act_ids, _lps = agent.act(context_token_ids=input_ids, n_tokens=env.action_token_budget, temperature=args.temperature, eos_token_ids=eos_token_ids)
        ...
        outcomes.append(NegotiationOutcome(deal=deal, score_a=score_a, score_b=score_b, items=items, values_a=values['agent_0'], values_b=values['agent_1']))
        if args.save_trajectories:
            from marlllm.trace_utils import make_episode_record
            agent_context_tokens = {aid: contexts[aid].get_context_ids() for aid in agents}
            ep_trace = {'deal': deal, 'score_a': score_a, 'score_b': score_b, 'items': list(items), 'values_a': list(values['agent_0']), 'values_b': list(values['agent_1'])}
            trajectories.append(make_episode_record(episode_idx=ep, agent_context_tokens=agent_context_tokens, tokenizer=agent_0.tokenizer, env_trace=ep_trace))
