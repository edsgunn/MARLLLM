...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Regenerate multi-agent episode traces from a saved checkpoint.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    p.add_argument(
        "--run-dir", required=True,
        help="Path to an experiment output directory that contains config.json "
             "and (optionally) experiment_config.yaml and checkpoints/.",
    )
    p.add_argument(
        "--checkpoint", default=None,
        help="Path to a .pt checkpoint file.  May be absolute or relative to "
             "--run-dir.  Defaults to checkpoints/latest.pt inside --run-dir.",
    )
    p.add_argument(
        "--no-checkpoint", action="store_true",
        help="Skip loading any checkpoint (run with randomly initialised or "
             "pre-trained base weights).",
    )
    p.add_argument(
        "--n-traces", type=int, default=10,
        help="Number of episodes to run and save as traces (default 10).",
    )
    p.add_argument(
        "--output-dir", default=None,
        help="Directory to write trace files into.  Defaults to "
             "<run-dir>/traces_regen/.",
    )
    p.add_argument(
        "--device", default=None,
        help="PyTorch device override (e.g. 'cpu', 'cuda:0').  "
             "If omitted the device from config.json is used.",
    )
    p.add_argument(
        "--seed", type=int, default=None,
        help="RNG seed for episode generation.  Defaults to the seed in config.json.",
    )
    p.add_argument(
        "--temperature", type=float, default=None,
        help="Sampling temperature.  Defaults to the value in config.json.",
    )
    # Environment overrides — read from experiment_config.yaml when present
...
def main() -> None:
    ...
    # YAML config has env-specific params (dialogue_turns, token_budget, etc.)
    yaml_cfg = _load_yaml_if_present(run_dir / "experiment_config.yaml")

    # ── Resolve parameters (CLI > YAML config > JSON config > hardcoded defaults)
    ...
    if lora_shared:
        ...
        agent_0 = LoRASharedBaseAgent(
            "agent_0", prompt_0, peft_model, "agent_0",
            tokenizer, device_0, keep_ref_model=False,
        )
        agent_1 = LoRASharedBaseAgent(
            "agent_1", prompt_1, peft_model, "agent_1",
            tokenizer, device_0, keep_ref_model=False,
        )

    elif shared_weights:
        print(f"Loading shared agent backbone: {model_name}  →  {device_0}")
        shared = IndependentAgent(
            agent_id="agent_0",
            character_prompt=prompt_0,
    ...
    if not args.no_checkpoint:
        ...
        if ckpt_path.exists():
            ...
            # Use a set to avoid double-loading when agent_0 and agent_1 are the
            # same object (shared_weights=True).
    ...
    print(f"Building DealOrNoDealEnv (dialogue_turns={dialogue_turns}, "
          f"token_budget={token_budget}, role_shuffle={role_shuffle})")
    ...
    # The Trainer is used only for episode collection (_collect_episodes_batched).
    # Point its output_dir at a scratch subdir so it doesn't pollute output_dir
    # with checkpoints/ or train.log.
    ...
    train_cfg = TrainingConfig(
        ...
        episodes_per_iter=args.n_traces,
        max_episode_tokens=max_ep_tok,
        num_iterations=1,
