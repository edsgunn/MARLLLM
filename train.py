...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--dtype', default='auto', help='torch_dtype for model loading: auto, float32, bfloat16, float16')
    p.add_argument('--device-map', default=None, help="device_map for from_pretrained, e.g. 'auto' to shard across GPUs")
    ...
    p.add_argument('--prompt', default='You are a counter that always outputs the next integer in sequence.', help='Character prompt for agent_0')
    if pre_args.config:
...
def main() -> None:
    ...
    config = TrainingConfig(model_name_or_path=args.model, character_prompts={'agent_0': args.prompt}, episodes_per_iter=args.rollouts, num_iterations=args.iters, lr=args.lr, log_every=args.log_every, checkpoint_every=args.checkpoint_every, output_dir=args.output_dir, device=args.device, seed=args.seed, kl_coef=args.kl_coef)
    ...
    agent = IndependentAgent(agent_id='agent_0', character_prompt=args.prompt, model_name_or_path=args.model, device=args.device, torch_dtype=torch_dtype, device_map=args.device_map, keep_ref_model=args.kl_coef > 0.0)
    ...
    loss = CCSMLoss()
    store = OnPolicyStore()
    trainer = Trainer(agents={'agent_0': agent}, env=env, loss=loss, tokeniser=tokeniser, store=store, config=config)
    start_iteration = 1
    if args.resume:
