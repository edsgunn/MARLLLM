...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Migrate one agent's adapter between population checkpoints.",
    )
    p.add_argument("--source-checkpoint", required=True,
                   help="Path to source population checkpoint dir (contains per-agent .pt files).")
    p.add_argument("--source-agent", required=True,
                   help="Agent slot name in source checkpoint (e.g. 'Silas Varnham').")
    p.add_argument("--target-checkpoint", required=True,
                   help="Path to target population checkpoint dir.")
    p.add_argument("--target-agent", default=None,
                   help="Agent slot in target to overwrite. Defaults to --source-agent.")
    p.add_argument("--output", required=True,
                   help="Output directory for the migrated checkpoint.")
    p.add_argument("--reset-optimizer", action="store_true",
                   help="Drop the target's optimizer state in the migrated copy "
                        "(recommended: optimizer momentum is incoherent post-migration).")
    return p.parse_args()
...
def _resolve_agent_file(ckpt_dir: Path, agent_slot: str) -> Path:
    ...
    if not meta_path.exists():
        raise FileNotFoundError(
            f"Neither {direct} nor {meta_path} found. Is this a per-agent checkpoint dir?"
        )
    ...
    if cand.exists():
        return cand
    raise FileNotFoundError(
        f"Could not find a .pt file for agent slot '{agent_slot}' in {ckpt_dir}. "
        f"Available files: {sorted(p.name for p in ckpt_dir.glob('*.pt'))}"
    )
...
def main() -> None:
    ...
    # 1. Copy every file from target into output (we will overwrite one file).
    for f in tgt_dir.iterdir():
        if f.is_file():
            shutil.copy2(f, out_dir / f.name)

    # 2. Build the migrated agent payload by combining source content with
    #    target adapter_name (so it loads cleanly into the target population).
    ...
    if src_type != tgt_type:
        sys.exit(
            f"Source agent_type ({src_type}) != target agent_type ({tgt_type}). "
            "Cross-type migration is not supported."
        )
    ...
    if src_type == "lora_shared":
        ...
        if src_adapter != tgt_adapter:
            adapter = {
                k.replace(f".{src_adapter}.", f".{tgt_adapter}."): v
                for k, v in adapter.items()
            }
            print(f"Adapter-name remap: {src_adapter} -> {tgt_adapter} ({len(adapter)} tensors)")
        new_payload = {
            "agent_id": target_agent,
            "agent_type": "lora_shared",
            "adapter_name": tgt_adapter,
            "adapter": adapter,
            "adapter_param_count": len(adapter),
            "value_head": src_payload["value_head"],
        }
    else:
        new_payload = dict(src_payload)
        new_payload["agent_id"] = target_agent

    # 3. Write the migrated agent file under the target's canonical name.
