...
def _load_factory_from_module(spec: str) -> Any:
    ...
    if ':' not in spec:
        sys.exit(f"--factory-module must be in the form 'path/to/module.py:FactoryClass', got: {spec!r}")
...
_DEFAULT_AGENTS: dict[str, list[str]] = {'round_robin': ['agent_0', 'agent_1'], 'dialogue': ['agent_0', 'agent_1'], 'haggling': ['merchant_0', 'merchant_1'], 'pub_coordination': ['alice', 'bob', 'carol', 'dave'], 'labor': ['worker_0', 'worker_1', 'worker_2', 'worker_3'], 'reality_show': ['player_0', 'player_1', 'player_2', 'player_3'], 'state_formation': ['villager_0', 'villager_1', 'villager_2', 'villager_3']}
...
def _build_scenario(args: argparse.Namespace) -> tuple[Any, list[str], Any]:
    ...
    from envs.concordia_scenarios import FreeDialogueScenario, HagglingScenario, LaborCollectiveActionScenario, PubCoordinationScenario, RealityShowScenario, RoundRobinScenario, StateFormationScenario
    scenario_name: str = args.scenario
    agent_names: list[str] = [a.strip() for a in args.agents.split(',')] if args.agents else _DEFAULT_AGENTS.get(scenario_name, ['agent_0', 'agent_1'])
    reward_fn = _load_reward_fn(getattr(args, 'reward_fn_module', None))
    if scenario_name == 'round_robin':
        scenario = RoundRobinScenario(context=args.context, num_turns=args.num_turns)
        return (scenario, agent_names, reward_fn)
    if not args.gm_model:
        sys.exit(f"--gm-model is required for scenario '{scenario_name}'.  Specify a HuggingFace model ID, or use --scenario round_robin for a dependency-free baseline.")
    ...
    gm_model = huggingface_model.HuggingFaceLanguageModel(model_name=args.gm_model, api_key=os.environ.get('HF_TOKEN') or None, dtype=gm_dtype)
    embedder = None
    if args.embedder:
    ...
    if scenario_name == 'dialogue':
        scenario = FreeDialogueScenario(gm_model=gm_model, embedder=embedder, context=args.context, num_turns=args.num_turns)
        return (scenario, agent_names, reward_fn)
    contest_map = {'haggling': (HagglingScenario, {'num_rounds': args.num_turns}), 'pub_coordination': (PubCoordinationScenario, {'num_players': len(agent_names)}), 'labor': (LaborCollectiveActionScenario, {'num_days': args.num_turns}), 'reality_show': (RealityShowScenario, {'num_rounds': args.num_turns}), 'state_formation': (StateFormationScenario, {})}
    ...
    if scenario_name == 'custom':
        ...
        return (scenario, agent_names, reward_fn)
    sys.exit(f"Unknown scenario: {scenario_name!r}.  Choose from: {', '.join(list(_DEFAULT_AGENTS) + ['custom'])}")

def parse_args() -> argparse.Namespace:
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument('--config', default=None)
    pre_args, _ = pre.parse_known_args()
    p = argparse.ArgumentParser(description='N-agent CCSM training with a Concordia environment.', formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__)
    p.add_argument('--config', default=None, help='Path to a YAML experiment config. All keys map to CLI flags; explicit CLI args override config file values.')
    p.add_argument('--scenario', default='round_robin', choices=['round_robin', 'dialogue', 'haggling', 'pub_coordination', 'labor', 'reality_show', 'state_formation', 'custom'], help='Which Concordia scenario to train in (default: round_robin).')
    p.add_argument('--agents', default=None, help="Comma-separated agent names, e.g. 'Alice,Bob,Carol'. Defaults vary per scenario (see _DEFAULT_AGENTS in this file).")
    p.add_argument('--num-turns', type=int, default=10, help='Scenario length: rounds for contest scenarios, acts for round_robin.')
    p.add_argument('--context', default='You are participating in a multi-agent discussion.', help='Opening observation sent to all agents at episode start (used by round_robin and dialogue scenarios).')
    p.add_argument('--factory-module', default=None, metavar='MODULE:CLASS', help="Load a custom scenario factory from a Python file, e.g. 'my_scenarios.py:MyFactory'.  Required when --scenario custom.")
    p.add_argument('--reward-fn-module', default=None, metavar='MODULE:FN', help="Load a custom reward function from a Python file, e.g. 'my_rewards.py:reward_fn'.  Signature: (final_state, agent_name) -> float.")
    p.add_argument('--gm-model', default=None, help='HuggingFace model ID for the Concordia GameMaster LLM. Not required for --scenario round_robin.')
    p.add_argument('--gm-device', default=None, help='Device for the GM model (defaults to next available CUDA device after the trained agents, or cpu).')
    p.add_argument('--embedder', default=None, help='sentence-transformers model ID for Concordia associative memory (e.g. sentence-transformers/all-MiniLM-L6-v2).  Required by contest scenarios that use semantic memory retrieval.')
    ...
    p.add_argument('--prompt', default='', help="Character prompt prepended to every agent's context. Leave empty to let the scenario define each agent's persona.")
    p.add_argument('--attn-impl', default=None, help="attn_implementation for from_pretrained (e.g. 'eager' for GPT-2).")
...
def main() -> None:
    ...
    for agent_name in agent_names:
        ...
        ag = IndependentAgent(agent_id=agent_name, character_prompt=args.prompt, model_name_or_path=args.model, device=dev, torch_dtype=torch_dtype, device_map=args.device_map, keep_ref_model=args.kl_coef > 0.0, gradient_checkpointing=args.gradient_checkpointing, lora_r=args.lora_r, lora_alpha=args.lora_alpha, lora_target_modules=lora_modules, compile_model=args.compile, attn_implementation=args.attn_impl)
        agents[agent_name] = ag
        character_prompts[agent_name] = args.prompt
    primary_agent = agents[agent_names[0]]
    print(f'Building ConcordiaEnv (token_budget={args.token_budget}, max_turns={args.max_env_turns})')
    env = ConcordiaEnv(simulation_factory=scenario_factory, agent_names=agent_names, tokenizer=primary_agent.tokenizer, action_token_budget=args.token_budget, max_turns=args.max_env_turns, reward_fn=reward_fn)
    config = TrainingConfig(model_name_or_path=args.model, character_prompts=character_prompts, episodes_per_iter=args.rollouts, max_episode_tokens=args.max_episode_tokens, num_iterations=args.iters, lr=args.lr, log_every=args.log_every, checkpoint_every=args.checkpoint_every, output_dir=args.output_dir, device=devices[agent_names[0]], seed=args.seed, kl_coef=args.kl_coef, grad_accum_steps=args.grad_accum, gradient_checkpointing=args.gradient_checkpointing, lora_r=args.lora_r, lora_alpha=args.lora_alpha, lora_target_modules=lora_modules)
